...
- Added one new permission (`shifts_date_correction`) specifically for Pay Period Setup's date-correction tool, rather than stretching an existing permission to cover something it wasn't really about.
- **Zero regression, actually proven, not assumed**: ran the complete existing test suite immediately after conversion — all 89 checks stayed green, including the specific test that verifies a Viewer account still can't fabricate an approved exception (the original documented vulnerability this function was built to close).
- **Proved real granularity, not just a relabeled admin/not-admin check**: created a custom role with *only* `staff_manage` and confirmed directly — it can edit staff records, but the identical account is correctly blocked from adding a location or adding a shift, since those need their own separate permissions it wasn't granted. This is the actual point of the whole system: fine-grained control that a single "isAdmin" boolean could never express.
- Found and fixed a small follow-on gap while doing this: the new `shifts_date_correction` permission wasn't in the Roles & Permissions page's checkbox list, meaning an admin couldn't actually grant it to a custom role through the UI even though the server understood it. Added it to the display list.
